# STATUS: EXPERIMENTAL
# Purpose: torch_per_label_softmax_global variant with an L2 anchor on the global softmax weights.
from pathlib import Path
import sys
import time

# Allow running as a script: `uv run benchmarks/torch_per_label_softmax_global_l2_anchor.py`
...
class PerLabelSoftmaxGlobalL2AnchorEnsemble(nn.Module):
    """
    Per-label ensemble with a softmax-constrained global mixture plus per-label residuals,
    with an explicit L2 anchor on the global weights.

    Parameters:
      - g_raw[m] (M,) with g = softmax(g_raw)
      - w_delta[m, l] (M, L)
      - bias[l] (L,)

    Anchor target:
      - g0[m] (M,) fixed, derived from dataset init weights if provided, else uniform.
    ...
    def __init__(
        ...
        *,
        ...
        if init_global is None:
            g0 = torch.full((self.n_models,), 1.0 / self.n_models, dtype=torch.float32)
        else:
            ...
            g0 = init_global.to(dtype=torch.float32).clone()
            s = float(g0.sum().item())
            if not np.isfinite(s) or s <= 0.0:
                raise ValueError("init_global must sum to a positive finite value")
            g0 = g0 / g0.sum()

        # Store the anchor target as a buffer (moves with device, not trainable).
        self.register_buffer("g0", g0)

        # Parameterize g via logits; initialize so softmax(g_raw) == g0.
        self.g_raw = nn.Parameter(torch.log(torch.clamp(g0, min=1e-12)))

        # Per-label residuals around g (init at 0)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = (x * w_eff).sum(dim=1) + self.bias
        return out

    def delta_l2(self) -> torch.Tensor:
        # Mean squared value (scale-invariant w.r.t. label count)
        return (self.w_delta**2).mean()

    def global_anchor_l2(self) -> torch.Tensor:
        # Mean squared deviation from the anchor target (mean over M)
        g = self.global_w()
        return ((g - self.g0) ** 2).mean()
...
# Best hyperparameters (copied from torch_per_label baseline for fair comparison)
BEST_LR = 0.003
...
# Explicit L2 penalty on w_delta (kept for comparability with prior variants)
LAMBDA_DELTA_L2 = 1e-3

# Explicit L2 anchor penalty on global simplex weights g vs g0
LAMBDA_GLOBAL_L2 = 1e-2
...
def train_and_evaluate(
    *,
    ...
    # Dataset-specific init weights (same behavior as torch_per_label baseline).
    ...
    model = PerLabelSoftmaxGlobalL2AnchorEnsemble(
        n_models=n_models,
        n_labels=n_labels,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for xb, yb in train_loader:
            ...
            loss = loss_bce + (LAMBDA_DELTA_L2 * model.delta_l2()) + (LAMBDA_GLOBAL_L2 * model.global_anchor_l2())
            loss.backward()
            optimizer.step()
        ...
        print(
            f"[lr={lr:g} wd={weight_decay:g} bs={batch_size} lambda_delta={LAMBDA_DELTA_L2:g} lambda_global={LAMBDA_GLOBAL_L2:g}] "
            f"Epoch {epoch:02d} | "
...
def main() -> None:
    ...
    model_name = f"torch_per_label_softmax_global_l2_anchor({','.join(ensemble_keys)})"
    ...
    print(
        ...
        f"lambda_delta={LAMBDA_DELTA_L2:g} | "
        f"lambda_global={LAMBDA_GLOBAL_L2:g}"
    )
