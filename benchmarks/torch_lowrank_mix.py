# STATUS: EXPERIMENTAL
# Purpose: Cross-label mixing (A1) only (no U@V per-label residual weights).
#
# Form:
#   s[b,l] = sum_m w_global[m] * x[b,m,l]
#   mix[b,:] = s[b,:] @ (P@Q)
#   out = clamp(s + mix + bias, [0,1])
...
def main() -> None:
    ...
    model_name = f"torch_lowrank_mix({','.join(ensemble_keys)})"
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        with _Timer() as t_train:
            for xb, yb in train_loader:
                ...
                loss_reg = lambda_mix * model.mix_l2() + lambda_bias * model.bias_l2()
        ...
        print(
            f"[mix_rank={mix_rank} lambda_mix={lambda_mix:g} lambda_bias={lambda_bias:g}] "
            f"Epoch {epoch:02d} | "
