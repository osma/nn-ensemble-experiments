# STATUS: EXPERIMENTAL
# Purpose: torch_mean_residual variant that bounds per-label residual weights with a smooth tanh clamp.
#
# Variant: torch_mean_residual_delta_tanh_clamp (see EXPERIMENTS.md)
#
# Rationale:
# - Keep global per-model weights as the default behavior.
# - Prevent extreme per-label residual weights by parameterizing residuals as:
#
#     delta_w = delta_max * tanh(delta_raw)
#
# - Initialize delta_raw = 0 so initial behavior is exactly global_w (same as baseline).
# - Train on raw logits with BCEWithLogitsLoss (no output clamp), optimizing ranking.
#
# Regularization:
# - L2 penalty on delta_raw (equivalently shrinks delta_w toward 0 near the origin)
# - L2 penalty on per-label bias
#
# Note:
# - This script intentionally mirrors benchmarks/torch_mean_residual.py and does not
#   introduce any new CLI parameters, to keep comparisons controlled.
from __future__ import annotations

from pathlib import Path
import sys

# Allow running as a script: `uv run benchmarks/torch_mean_residual_delta_tanh_clamp.py`
...
# Variant-specific constant (no CLI flag to keep comparisons controlled)
DELTA_MAX = 0.25  # bounds per-label residual magnitude: delta_w in [-DELTA_MAX, +DELTA_MAX]

# Reproducibility
TRAIN_SEED = 0


class MeanResidualDeltaTanhClampEnsemble(nn.Module):
    """
    Mean-like ensemble with bounded per-label residual weights (tanh clamp).

    Form:
      logits[b, l] = sum_m (global_w[m] + delta_w[m, l]) * x[b, m, l] + bias[l]
    ...
    def __init__(self, n_models: int, n_labels: int, init_global: torch.Tensor | None):
        ...
        # Raw per-label residual weights initialized to 0. Learnable.
        # Effective delta is bounded via tanh.
        self.delta_raw = nn.Parameter(torch.zeros((n_models, n_labels), dtype=torch.float32))
    ...
    def delta_w(self) -> torch.Tensor:
        # (M, L), bounded in [-DELTA_MAX, +DELTA_MAX]
        return float(DELTA_MAX) * torch.tanh(self.delta_raw)
    ...
    def delta_l2(self) -> torch.Tensor:
        # Regularize raw residuals directly; keeps optimization well-behaved even when saturated.
        # Mean squared residual for scale-invariant regularization.
        return (self.delta_raw ** 2).mean()
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="L2 shrinkage strength for per-label residual weights (delta_raw)",
    )
    parser.add_argument(
        "--lambda-bias",
        type=float,
    ...
    parser.add_argument(
        "--print-delta",
        action="store_true",
        help="Print delta diagnostics (delta_l2 and per-model mean |delta|) each epoch",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    model_name = f"torch_mean_residual_delta_tanh_clamp({','.join(ensemble_keys)})"
    ...
    model = MeanResidualDeltaTanhClampEnsemble(
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if print_delta or debug:
            with torch.no_grad():
                ...
                delta = model.delta_w().detach()
                mean_abs_delta_per_model = delta.abs().mean(dim=1).cpu().numpy()
        ...
        if debug:
            with torch.no_grad():
                ...
                delta_raw = model.delta_raw.detach()
            ...
            dr_stats = _tensor_stats_all(delta_raw)
            dw_stats = _tensor_stats_all(delta_w)
            ...
            extra = (
                ...
                f"    delta_raw:  mean={dr_stats['mean']:.6e} std={dr_stats['std']:.6e} "
                f"min={dr_stats['min']:.6e} p50={dr_stats['p50']:.6e} max={dr_stats['max']:.6e}\n"
                f"    delta_w:    mean={dw_stats['mean']:.6e} std={dw_stats['std']:.6e} "
                f"min={dw_stats['min']:.6e} p50={dw_stats['p50']:.6e} max={dw_stats['max']:.6e}\n"
