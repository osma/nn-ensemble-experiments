# STATUS: EXPERIMENTAL
# Purpose: torch_mean_residual variant with bias reparameterized as global + per-label residual.
#
# Rationale:
# - The baseline torch_mean_residual uses a free per-label bias vector. This variant mirrors the
#   "residual" design of weights by splitting bias into:
#       bias[l] = bias_global + bias_delta[l]
# - bias_delta is strongly regularized toward 0, while bias_global acts as a dataset-wide intercept.
#
# Form:
#   logits[b, l] = sum_m (w_global[m] + delta_w[m, l]) * x[b, m, l] + (bias_global + bias_delta[l])
#
# Notes:
# - No new CLI flags: uses the same arguments as torch_mean_residual.py.
# - Regularization uses existing lambdas:
#     - lambda_bias applies to bias_delta only (not bias_global).
from __future__ import annotations

from pathlib import Path
import sys

# Allow running as a script: `uv run benchmarks/torch_mean_residual_bias_residual.py`
...
LAMBDA_BIAS_L2 = 1e-3  # shrinkage for per-label bias residual (bias_delta)

# Reproducibility
TRAIN_SEED = 0


class MeanResidualBiasResidualEnsemble(nn.Module):
    """
    Mean-like ensemble with per-label residual weights and bias reparameterized as:
        bias[l] = bias_global + bias_delta[l]
    ...
    def __init__(self, n_models: int, n_labels: int, init_global: torch.Tensor | None):
        ...
        # - bias_global: dataset-wide scalar intercept (not regularized by bias_l2)
        # - bias_delta: per-label bias residual (regularized by bias_l2)
        self.bias_global = nn.Parameter(torch.zeros((), dtype=torch.float32))  # scalar
        self.bias_delta = nn.Parameter(torch.zeros((n_labels,), dtype=torch.float32))  # (L,)
    ...
    def effective_bias(self) -> torch.Tensor:
        # (L,)
        return self.bias_global + self.bias_delta
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        logits = (x * w_eff.unsqueeze(0)).sum(dim=1) + self.effective_bias()  # (B, L)
        return logits
    ...
    def bias_l2(self) -> torch.Tensor:
        # Regularize only the per-label bias residual.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="L2 shrinkage strength for per-label bias residual (bias_delta)",
    )
    parser.add_argument(
        "--print-delta",
        action="store_true",
    ...
    model_name = f"torch_mean_residual_bias_residual({','.join(ensemble_keys)})"
    ...
    model = MeanResidualBiasResidualEnsemble(
        n_models=n_models,
        n_labels=n_labels,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if print_delta or debug:
            with torch.no_grad():
                ...
                mean_abs_bias_delta = float(model.bias_delta.detach().abs().mean().cpu().item())
                max_abs_bias_delta = float(model.bias_delta.detach().abs().max().cpu().item())
                bias_global = float(model.bias_global.detach().cpu().item())
            ...
            diag = (
                ...
                f"bias_global={bias_global:.3e} "
                f"mean_abs_bias_delta={mean_abs_bias_delta:.3e} "
                f"max_abs_bias_delta={max_abs_bias_delta:.3e}"
            )
        ...
        if debug:
            with torch.no_grad():
                ...
                bias_delta = model.bias_delta.detach()
                bias_global_t = model.bias_global.detach()

                subset_scores = train_scores_eval.detach()

            w_stats = _tensor_stats_1d(w_global)
            d_stats = _tensor_stats_all(delta_w)
            bd_stats = _tensor_stats_1d(bias_delta)
            s_stats = _tensor_stats_all(subset_scores)
            bg = float(bias_global_t.detach().cpu().item())
            ...
            extra = (
                ...
                f"    bias_delta:   mean={bd_stats['mean']:.6e} std={bd_stats['std']:.6e} "
                f"min={bd_stats['min']:.6e} p50={bd_stats['p50']:.6e} max={bd_stats['max']:.6e}\n"
                f"    bias_global:  {bg:.6e}\n"
