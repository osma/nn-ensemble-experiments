# STATUS: EXPERIMENTAL
# Purpose: Simplified NN ensemble - Conv1d only (no MLP), clamp output.
#
# Model:
#   out = clamp(Conv1d over sources + per-label bias, [0,1])
#
# Training:
...
class SimpleNNModel(nn.Module):
    def __init__(self, source_dim: int, output_dim: int):
        super().__init__()
        self.model_config = {
            "source_dim": int(source_dim),
            "output_dim": int(output_dim),
        }
        self.conv = nn.Conv1d(source_dim, 1, 1, bias=True)
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        return torch.clamp(out[:, 0, :], min=0.0, max=1.0)
...
def main() -> None:
    ...
    model_name = f"torch_nn_simple({','.join(e3)})"
    ...
    model = SimpleNNModel(source_dim=n_models, output_dim=n_labels).to(DEVICE)
