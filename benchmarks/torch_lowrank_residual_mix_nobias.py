# STATUS: EXPERIMENTAL
# Purpose: Low-rank residual weights (U@V) + A1 cross-label mixing (P@Q), but WITHOUT bias.
...
class LowRankResidualMixNoBias(nn.Module):
    def __init__(
        self,
        *,
        n_models: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.clamp(out_base + mix, 0.0, 1.0)
...
def main() -> None:
    ...
    model_name = f"torch_lowrank_residual_mix_nobias({','.join(ensemble_keys)})"
    ...
    model = LowRankResidualMixNoBias(
        n_models=n_models,
        n_labels=n_labels,
        rank=rank,
        mix_rank=mix_rank,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        with _Timer() as t_train:
            for xb, yb in train_loader:
                ...
                loss_reg = lambda_uv * model.uv_l2() + lambda_mix * model.mix_l2()
        ...
        print(
            f"[rank={rank} mix_rank={mix_rank} lambda_uv={lambda_uv:g} lambda_mix={lambda_mix:g}] "
            f"Epoch {epoch:02d} | "
