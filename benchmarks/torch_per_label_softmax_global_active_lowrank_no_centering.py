...
# Simplification 6: Remove per-sample centering from the low-rank delta.
#
# Follows the architecture of torch_per_label_softmax_global_active_lowrank, but
# removes the delta = delta - delta.mean(dim=1) step. This tests whether the
# low-rank constraint is sufficient to prevent the "centering" regression
# observed in the MLP variants.
...
class ActiveLowRankEnsemble(nn.Module):
    ...
    def get_lowrank_delta(self, x_active: torch.Tensor, base_logits_active: torch.Tensor) -> torch.Tensor:
        ...
        # Simplification 6: Remove per-sample centering
    ...
    *,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for xb, yb in train_loader:
            ...
            loss_l2 = LAMBDA_DELTA_L2 * model.delta_l2()
            loss = loss_bce + loss_l2
            loss.backward()
            optimizer.step()
            
            total_loss_bce += float(loss_bce.item())
            total_loss_l2 += float(loss_l2.item())
            n_batches += 1

        avg_loss_bce = total_loss_bce / max(1, n_batches)
        avg_loss_l2 = total_loss_l2 / max(1, n_batches)
        ...
        print(
            f"Epoch {epoch:02d} | loss_bce={avg_loss_bce:.6f} l2={avg_loss_l2:.6f} grad={total_grad_norm:.4f} | "
