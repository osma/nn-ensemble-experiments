# STATUS: EXPERIMENTAL
# Purpose: Low-rank residual weights (U@V) + A1 cross-label mixing (P@Q) with a learnable
# scalar alpha_mix multiplying the mixing contribution.
...
class LowRankResidualMixTemp(nn.Module):
    def __init__(
        self,
        *,
        n_models: int,
        ...
        # Start near "no mixing"
        self.alpha_mix = nn.Parameter(torch.tensor(0.0, dtype=torch.float32))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out_base = (x * w_eff.unsqueeze(0)).sum(dim=1)

        s = (x * self.global_w[None, :, None]).sum(dim=1)
        mix = (s @ self.P) @ self.Q

        out_lin = out_base + self.alpha_mix * mix + self.bias
...
def main() -> None:
    ...
    model_name = f"torch_lowrank_residual_mix_temp({','.join(ensemble_keys)})"
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        alpha_mix = float(model.alpha_mix.detach().cpu().item())
        print(
            f"[rank={rank} mix_rank={mix_rank} alpha_mix={alpha_mix:.4f}] "
            f"Epoch {epoch:02d} | "
