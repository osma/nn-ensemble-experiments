# STATUS: EXPERIMENTAL
# Purpose: Per-label linear ensemble with softmax-constrained global weights and multiplicative per-label scaling.
from pathlib import Path
import sys
import time

# Allow running as a script: `uv run benchmarks/torch_per_label_global_times_scale.py`
...
class PerLabelGlobalTimesScaleEnsemble(nn.Module):
    """
    torch_per_label_global_times_scale

    Parameters:
      - g_raw[m] (M,), with g = softmax(g_raw)
      - scale_raw[m, l] (M, L), with scale = 1 + tanh(scale_raw) in (0, 2)
      - bias[l] (L,)

    Effective weights:
      w_eff[m, l] = g[m] * scale[m, l]

    Forward:
      logits[b, l] = sum_m w_eff[m, l] * x[b, m, l] + bias[l]

    Input:
      x: (batch, M, L) log1p-preprocessed non-negative base scores

    Output:
      (batch, L) raw logits (no sigmoid)
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Parameterize global weights via logits; initialize so softmax(g_raw) ~= init.
        # Add epsilon to avoid log(0) if init contains exact zeros.
        eps = 1e-12
        self.g_raw = nn.Parameter(torch.log(torch.clamp(init, min=eps)))

        # Per-model, per-label scaling (initialized to 0 => scale ~= 1)
        self.scale_raw = nn.Parameter(torch.zeros(self.n_models, self.n_labels))
    ...
    def scale(self) -> torch.Tensor:
        # (M, L) in (0, 2)
        return 1.0 + torch.tanh(self.scale_raw)

    def effective_w(self) -> torch.Tensor:
        # (M, L)
        return self.global_w()[:, None] * self.scale()
...
# Best hyperparameters (from tuning) - start with torch_per_label defaults
BEST_LR = 0.003
...
def train_and_evaluate(
    *,
    ...
    model = PerLabelGlobalTimesScaleEnsemble(
        n_models=n_models,
        n_labels=n_labels,
...
def main():
    ...
    model_name = f"torch_per_label_global_times_scale({','.join(ensemble_keys)})"
