...
# Simplification 4: No base channel in low-rank mixer.
#
# Removes the base_logits channel from the low-rank mixer, using only the raw
# model inputs. This eliminates the circular dependency and simplifies the mixer.
...
class ActiveLowRankEnsemble(nn.Module):
    def __init__(
        ...
        *,
        ...
        # C = M raw logits (NO base channel in this simplification)
...
def train_and_evaluate(
    *,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            f"Epoch {epoch:02d} | loss={float(last_loss or 0.0):.6f} | grad_norm={avg_grad_norm:.4f} | "
...
def main() -> None:
    ...
    model_name = f"torch_per_label_softmax_global_active_lowrank_no_base_ch({','.join(ensemble_keys)})"
