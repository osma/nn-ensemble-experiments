# STATUS: EXPERIMENTAL
# Purpose: torch_mean_residual variant with frequency-weighted per-label residual shrinkage.
#
# Rationale:
# - torch_per_label tends to win because it can learn different mixtures per label.
# - torch_mean_residual already has per-label flexibility via delta_w, but a single global
#   lambda_delta is a blunt regularizer for large label spaces.
# - This variant keeps the baseline model form but weights the delta L2 penalty per label,
#   shrinking rare labels harder and frequent labels less.
#
# Form:
#   logits[b, l] = sum_m (w_global[m] + delta_w[m, l]) * x[b, m, l] + bias[l]
#
# Regularization change (only):
#   freq[l]  = number of positive train examples for label l
#   alpha[l] = 1 / sqrt(freq[l] + 1), normalized so mean(alpha)=1
#   delta_l2 = mean_{m,l}( alpha[l] * delta_w[m,l]^2 )
#
# Notes:
# - No new CLI flags; the script is intended to be run like other variants.
from __future__ import annotations

from pathlib import Path
import sys

# Allow running as a script: `uv run benchmarks/torch_mean_residual_freq_weighted_delta.py`
...
def _compute_freq_weight_alpha(y_true_train) -> torch.Tensor:
    """
    Compute per-label regularization weights alpha[l] from training label frequencies.

    alpha[l] = 1 / sqrt(freq[l] + 1), then normalized so mean(alpha)=1.

    Args:
        y_true_train: CSR matrix of shape (N, L) with binary ground truth.

    Returns:
        alpha: torch.float32 tensor of shape (L,) on CPU.
    """
    # freq is count of non-zeros per column (positives per label)
    # Using CSR.getnnz(axis=0) is efficient and returns np.ndarray.
    freq = np.asarray(y_true_train.getnnz(axis=0), dtype=np.float64)  # (L,)
    alpha = 1.0 / np.sqrt(freq + 1.0)

    # Normalize to keep the average penalty scale comparable to the baseline delta_l2.
    mean_alpha = float(alpha.mean())
    if not np.isfinite(mean_alpha) or mean_alpha <= 0.0:
        raise ValueError("Invalid alpha normalization (non-finite or non-positive mean).")
    alpha = alpha / mean_alpha

    return torch.from_numpy(alpha.astype(np.float32, copy=False))


class MeanResidualEnsembleFreqWeightedDelta(nn.Module):
    """
    Mean-like ensemble with per-label residual weights, with frequency-weighted delta L2.
    ...
    def __init__(
        ...
        *,
        ...
        if alpha.ndim != 1 or int(alpha.shape[0]) != n_labels:
            raise ValueError(f"alpha must have shape ({n_labels},), got {tuple(alpha.shape)}")
        if alpha.dtype not in (torch.float32, torch.float64):
            raise ValueError("alpha must be a float tensor")
        # Keep alpha as a non-trainable buffer so it is moved with .to(device) and saved in state_dict.
        self.register_buffer("alpha", alpha.to(dtype=torch.float32).clone())
    ...
    def delta_l2(self) -> torch.Tensor:
        """
        Frequency-weighted mean squared residual.

        Returns:
            Scalar tensor: mean_{m,l}( alpha[l] * delta_w[m,l]^2 )
        """
        # alpha: (L,), delta_w: (M, L) -> broadcast to (M, L)
        return ((self.delta_w ** 2) * self.alpha.unsqueeze(0)).mean()
...
def main() -> None:
    ...
    model_name = f"torch_mean_residual_freq_weighted_delta({','.join(ensemble_keys)})"
    ...
    # Compute per-label regularization weights alpha from training label frequencies.
    alpha = _compute_freq_weight_alpha(y_train_true)  # (L,), CPU
    ...
    model = MeanResidualEnsembleFreqWeightedDelta(
        n_models=n_models,
        n_labels=n_labels,
