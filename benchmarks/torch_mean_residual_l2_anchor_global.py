# STATUS: EXPERIMENTAL
# Purpose: torch_mean_residual variant with L2 anchor regularization on global weights.
#
# Change vs baseline:
#   Add an explicit penalty anchoring global_w to the dataset-provided init weights w0:
#       lambda_global * mean((global_w - w0)**2)
#
# Notes:
# - No new CLI flags: lambda_global is a module constant for controlled comparison.
# - Uses the same training loop, early stopping, and metrics as torch_mean_residual.py.
from __future__ import annotations

from pathlib import Path
import sys

# Allow running as a script: `uv run benchmarks/torch_mean_residual_l2_anchor_global.py`
...
# Variant hyperparameter: L2 anchor on global_w toward w0.
# Kept as a constant (no CLI param) for controlled comparisons.
LAMBDA_GLOBAL_L2_ANCHOR = 1e-2

# Reproducibility
TRAIN_SEED = 0


class MeanResidualEnsembleL2AnchorGlobal(nn.Module):
    """
    Mean-like ensemble with per-label residual weights and an L2 anchor on global weights.
    ...
    def __init__(self, n_models: int, n_labels: int, init_global: torch.Tensor | None):
        ...
        # Save w0 for the anchor penalty. Not trainable.
        self.register_buffer("w0", w0)  # (M,)

        # Global per-model weights (shared over labels). Learnable.
        self.global_w = nn.Parameter(w0.clone())  # (M,)
    ...
    def global_anchor_l2(self) -> torch.Tensor:
        # Mean squared deviation from the initialization weights.
        # Use mean() to keep scale comparable across different n_models.
        return ((self.global_w - self.w0) ** 2).mean()
...
def main() -> None:
    ...
    model_name = f"torch_mean_residual_l2_anchor_global({','.join(ensemble_keys)})"
    ...
    model = MeanResidualEnsembleL2AnchorGlobal(
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for xb, yb in train_loader:
            ...
            loss_reg_global = LAMBDA_GLOBAL_L2_ANCHOR * model.global_anchor_l2()
        ...
        if print_delta or debug:
            with torch.no_grad():
                ...
                global_drift_l2 = float(((model.global_w - model.w0) ** 2).mean().detach().cpu().item())
            ...
            diag = (
                ...
                f"global_drift_l2={global_drift_l2:.6e}"
            )
