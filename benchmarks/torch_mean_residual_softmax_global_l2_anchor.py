# STATUS: EXPERIMENTAL
# Purpose: torch_mean_residual variant that combines:
#   1) softmax-constrained global weights (convex combination)
#   2) explicit L2 anchor penalty to dataset-provided init weights
#
# Form:
#   global_w = softmax(global_logits)              # (M,)
#   w_eff[m, l] = global_w[m] + delta_w[m, l]      # (M, L)
#   logits[b, l] = sum_m w_eff[m, l] * x[b, m, l] + bias[l]
#
# Regularization:
#   - lambda_delta * mean(delta_w^2)
#   - lambda_bias  * mean(bias^2)
#   - lambda_global * mean((global_w - w0)^2)   where w0 is init weights (or uniform)
#
# CLI: intentionally identical to benchmarks/torch_mean_residual.py (no new flags)
from __future__ import annotations

from pathlib import Path
import sys

# Allow running as a script: `uv run benchmarks/torch_mean_residual_softmax_global_l2_anchor.py`
...
# Keep CLI identical to torch_mean_residual.py:
...
# Anchor strength (no CLI flag; keep controlled and comparable).
# Chosen to match the typical scale used in the dedicated anchor variant.
...
class MeanResidualSoftmaxGlobalL2AnchorEnsemble(nn.Module):
    """
    Mean-like ensemble with:
      - global weights constrained by softmax
      - per-label residual weights (additive)
      - per-label bias
      - explicit L2 penalties for delta/bias + L2 anchor to init weights
    ...
    def __init__(self, n_models: int, n_labels: int, init_global: torch.Tensor | None):
        ...
        # Keep the initialization identical-in-effect to torch_mean_residual_softmax_global:
        # Choose logits so that softmax(logits) == w0 (up to numerical precision).
        # Since w0 sums to 1 and w0_i > 0, logits = log(w0) works (softmax invariant to constants).
        eps = 1e-12
        w0_safe = torch.clamp(w0, min=eps)
        self.global_logits = nn.Parameter(torch.log(w0_safe))  # (M,)

        # Register the anchor target as a buffer (moves with .to(device), saved in state_dict)
        self.register_buffer("w0", w0)  # (M,)
    ...
    def global_anchor_l2(self) -> torch.Tensor:
        # Mean squared deviation of convex global weights from w0.
        w = self.global_w()
...
def main() -> None:
    ...
    model_name = f"torch_mean_residual_softmax_global_l2_anchor({','.join(ensemble_keys)})"
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if print_delta or debug:
            with torch.no_grad():
                ...
                global_anchor_l2 = float(model.global_anchor_l2().detach().cpu().item())
            ...
            diag = (
                ...
                f"global_anchor_l2={global_anchor_l2:.6e}"
            )
