# STATUS: EXPERIMENTAL
# Purpose: torch_per_label variant with weights reparameterized as global + per-label delta.
from pathlib import Path
import sys
import time

# Allow running as a script: `uv run benchmarks/torch_per_label_global_plus_delta.py`
...
class PerLabelGlobalPlusDeltaEnsemble(nn.Module):
    """
    torch_per_label_global_plus_delta

    Reparameterize per-label weights as:
    ...
    Input:
        x: (batch, M, L) log1p-preprocessed base scores (non-negative)
    ...
    def __init__(
        ...
        *,
        ...
        if init_global is None:
            ...
            init = init_global.to(dtype=torch.float32).clone()
            s = float(init.sum().item())
            if not np.isfinite(s) or s <= 0.0:
                raise ValueError("init_global must sum to a positive finite value")
            init = init / init.sum()

        # Global per-model weights (M,)
        self.w_global = nn.Parameter(init)

        # Per-model, per-label residuals (M, L), initialized at 0
    ...
    def effective_w(self) -> torch.Tensor:
        # (M, 1) + (M, L) -> (M, L)
        return self.w_global[:, None] + self.w_delta
...
# Best hyperparameters (from tuning) — copied from torch_per_label baseline.
BEST_LR = 0.003
...
def train_and_evaluate(
    *,
    ...
    """
    Clone of torch_per_label training loop, selecting best snapshot by
    TRAIN subset NDCG@1000 (early stopping metric).
    """
    if batch_size < 1:
    ...
    model = PerLabelGlobalPlusDeltaEnsemble(
        n_models=n_models,
        n_labels=n_labels,
...
def main():
    ...
    model_name = f"torch_per_label_global_plus_delta({','.join(ensemble_keys)})"
    ...
    print(
        ...
        f"train_ndcg@1000(subset)={float(result['best_metric']):.6f}"
    )
    print(
        "Best test metrics | "
