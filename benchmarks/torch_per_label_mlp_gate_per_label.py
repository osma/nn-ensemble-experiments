# STATUS: EXPERIMENTAL
# Purpose: Variant of torch_per_label_mlp.
...
#            make small cross-label adjustments.
#
# Variant: gate per active label (vector gate) instead of a single scalar gate.
#   - Baseline gate: gate = DELTA_GATE_MAX * sigmoid(raw_gate) (scalar)
#   - This variant: gate[l] = DELTA_GATE_MAX * sigmoid(raw_gate[l]) (for each active label)
#
# Keeps baseline multiplicative combination rule on active labels:
#   out_active = base_active * (1 + gate * delta_active)
#
# Debug:
#   - Always prints gate stats, residual delta stats, parameter stats, and timing each epoch.
...
# Allow running as a script: `uv run benchmarks/torch_per_label_mlp_gate_per_label.py`
...
class TwoStagePerLabelMLPGatePerLabel(nn.Module):
    """
    Wrapper model:
      - frozen base per-label ensemble in logits space
      - trainable residual MLP on active labels only
      - per-active-label bounded gate vector (learned)
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Learnable, bounded per-label gate vector:
        #   gate[l] = DELTA_GATE_MAX * sigmoid(raw_gate[l])
        ...
        self.raw_delta_gate = nn.Parameter(
            torch.full((self.n_active,), raw_init, dtype=torch.float32)
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Match baseline: center per sample to constrain residual to redistributing within active labels.
        delta_active = delta_active - delta_active.mean(dim=1, keepdim=True)

        gate = self.delta_gate()  # (L_active,)
        out = base_logits.clone()

        # Multiplicative combination (baseline form), with per-label gate:
        #   out_active = base_active * (1 + gate[l] * delta)
        out_active = base_logits_active * (1.0 + delta_active * gate.unsqueeze(0))
...
def main() -> None:
    ...
    model_name = f"torch_per_label_mlp_gate_per_label({','.join(e3)})"
    ...
    base = PerLabelWeightedEnsemble(
        n_models=n_models, n_labels=n_labels, init_model_weights=init_weights_s1
    ).to(DEVICE)
    ...
    # -----------------
    print("\n====================")
    print("Stage 2: residual MLP (active labels only) + per-label gate")
    print("====================")

    model = TwoStagePerLabelMLPGatePerLabel(
        base=base,
    ...
    for epoch in range(1, EPOCHS_STAGE2 + 1):
        ...
        with torch.no_grad():
            gate_vec = model.delta_gate().detach().cpu()
            gate_stats = _tensor_stats(gate_vec)
            ...
            if model.n_active == 0:
                ...
                gate_dbg = model.delta_gate()  # (L_active,)
                delta_g_stats = _tensor_stats((delta_dbg * gate_dbg.unsqueeze(0)).detach().cpu())
        ...
        print(
            ...
            f"gate_vec: {_fmt_stats(gate_stats)} gate_max={DELTA_GATE_MAX:g} delta_clamp={DELTA_CLAMP} "
