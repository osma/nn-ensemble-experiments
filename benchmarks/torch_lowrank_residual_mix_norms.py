# STATUS: EXPERIMENTAL
# Purpose: Low-rank residual (U@V) + A1 mixing (P@Q), but normalizes s per sample:
#   s_norm = s / (||s||_2 + eps)
# before computing mix. This can stabilize training across datasets.
...
class LowRankResidualMixNormS(nn.Module):
    def __init__(
        self,
        *,
        n_models: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        denom = torch.norm(s, p=2, dim=1, keepdim=True).clamp_min(1e-6)
        s_norm = s / denom

        mix = (s_norm @ self.P) @ self.Q
...
def main() -> None:
    ...
    model_name = f"torch_lowrank_residual_mix_norms({','.join(ensemble_keys)})"
