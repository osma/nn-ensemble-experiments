# STATUS: EXPERIMENTAL
# Purpose: torch_per_label_softmax_global variant with softmax-constrained global weights + per-label residuals,
#          but restoring global scale freedom via a learned positive scalar s = exp(log_s).
from pathlib import Path
import sys
import time

# Allow running as a script: `uv run benchmarks/torch_per_label_softmax_global_scale.py`
...
class PerLabelSoftmaxGlobalScaleEnsemble(nn.Module):
    """
    Per-label ensemble with a softmax-constrained global mixture, global scale freedom, plus per-label residuals.

    Parameters:
      - g_raw[m] (M,) with g = softmax(g_raw)
      - log_s (scalar) with s = exp(log_s) > 0
      - w_delta[m, l] (M, L)
      - bias[l] (L,)
    ...
    def __init__(
        ...
        *,
        ...
        if init_global is None:
            ...
            ssum = float(g.sum().item())
            if not np.isfinite(ssum) or ssum <= 0.0:
                raise ValueError("init_global must sum to a positive finite value")
            g = g / g.sum()

        # Parameterize g via logits; initialize so softmax(g_raw) == g.
        self.g_raw = nn.Parameter(torch.log(torch.clamp(g, min=1e-12)))

        # Global positive scale (initialize at s=1.0 by default)
        if not np.isfinite(float(init_s)) or float(init_s) <= 0.0:
            raise ValueError("init_s must be positive and finite")
        self.log_s = nn.Parameter(torch.tensor(float(np.log(init_s)), dtype=torch.float32))

        # Per-label residuals around s*g (init at 0)
    ...
    def effective_w(self) -> torch.Tensor:
        # (M, 1) + (M, L) -> (M, L)
        return (self.s() * self.global_w())[:, None] + self.w_delta
...
def train_and_evaluate(
    *,
    ...
    model = PerLabelSoftmaxGlobalScaleEnsemble(
        n_models=n_models,
        n_labels=n_labels,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            f"[lr={lr:g} wd={weight_decay:g} bs={batch_size} lambda_delta={LAMBDA_DELTA_L2:g}] "
            f"Epoch {epoch:02d} | "
...
def main() -> None:
    ...
    model_name = f"torch_per_label_softmax_global_scale({','.join(ensemble_keys)})"
