...
# Variant: simplify_no_clamp (Simplification 9)
#
# Removes the tanh clamp from the low-rank delta. The gate alone, plus
# weight decay on the low-rank parameters, should be sufficient to
# prevent instability.
...
class ActiveLowRankEnsemble(nn.Module):
    ...
    def get_lowrank_delta(self, x_active: torch.Tensor, base_logits_active: torch.Tensor) -> torch.Tensor:
        ...
        # Simplification 9: Remove tanh clamp
        # DELTA_CLAMP = 0.5
...
def _delta_stats(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            sum_max += float(delta.abs().max().item())
            n_abs += int(a.numel())
    ...
    mean_max = sum_max / len(loader)
    if not samples:
        return float(mean_abs), 0.0, float(mean_max)

    v = torch.cat(samples, dim=0)
    v, _ = torch.sort(v)
    q_idx = min(int(round(0.95 * (v.numel() - 1))), v.numel() - 1)
    p95_abs = float(v[q_idx].item())
    return float(mean_abs), float(p95_abs), float(mean_max)
...
def train_and_evaluate(
    *,
    ...
    base_params = [model.g_raw, model.w_delta, model.bias]
    lowrank_params = list(model.lowrank.parameters())
    gate_params = [model.raw_gate]

    optimizer = optim.AdamW(
        [
            {"params": base_params, "lr": BEST_LR, "weight_decay": BEST_WEIGHT_DECAY},
            {"params": lowrank_params, "lr": LOWRANK_LR, "weight_decay": LOWRANK_WEIGHT_DECAY},
            {"params": gate_params, "lr": LOWRANK_LR, "weight_decay": 0.0},
        ],
        eps=1e-8,
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for xb, yb in train_loader:
            ...
            # Debug: Capture grad norms
            with torch.no_grad():
                grad_norm_base += nn.utils.clip_grad_norm_(base_params, 1e9).item()
                grad_norm_lowrank += nn.utils.clip_grad_norm_(lowrank_params, 1e9).item()
                grad_norm_gate += nn.utils.clip_grad_norm_(gate_params, 1e9).item()
        ...
        avg_grad_base = grad_norm_base / n_batches
        avg_grad_lowrank = grad_norm_lowrank / n_batches
        avg_grad_gate = grad_norm_gate / n_batches
        ...
        delta_mean_abs, delta_p95_abs, delta_max_abs = _delta_stats(model, train_eval_loader, DEVICE)
        with torch.no_grad():
        ...
        print(
            ...
            f"test_ndcg@10={test_metrics['ndcg@10']:.6f} f1@5={test_metrics['f1@5']:.6f} | "
            f"gate={gate_val:.4f} delta_mean={delta_mean_abs:.6f} max={delta_max_abs:.6f} | "
            f"grad_base={avg_grad_base:.4f} grad_lr={avg_grad_lowrank:.4f} | "
            f"U_norm={u_norm:.2f} V_norm={v_norm:.2f} W_norm={w_norm:.2f} | "
...
def main() -> None:
    ...
    model_name = f"torch_per_label_softmax_global_active_lowrank_no_clamp({','.join(ensemble_keys)})"
