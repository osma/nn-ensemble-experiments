...
# but only learns label-specific corrections for labels that are "active" in the
# training data (appear in train truth and/or in any train predictor nnz).
#
# Motivation:
#   - Reduce compute/parameters by avoiding huge (M*L) MLP over all labels.
#   - Avoid wasting capacity on labels with no training signal.
#   - Preserve cross-label adjustments where learning is possible (active labels).
#
# Model:
#   mean_all = Conv1d over sources (initialized to dataset init weights if available)
#   delta_active = small MLP over flattened (sources x active_labels) inputs
#   out_active = clamp(mean_active + delta_active, [0,1])
#   out_inactive = clamp(mean_inactive, [0,1])   # no cross-label effects for inactive labels
#   out = stitch active/inactive into full label space
#
# Training:
#   - BCELoss on probabilities
#   - Early stopping by train subset NDCG@1000 (same as torch_nn)
#   - Default max epochs reduced to 12
#
# Diagnostics:
#   - Prints label activity stats and sparsity stats
#   - Prints conv weights, delta stats, and train/test metrics each epoch
...
# Allow running as a script: `uv run benchmarks/torch_nn_split.py`
...
class NNSplitEnsembleModel(nn.Module):
    def __init__(
        self,
        *,
        source_dim: int,
        ...
        # Global mean-like mixer for all labels.
        self.conv = nn.Conv1d(self.source_dim, 1, 1, bias=False)

        # Small MLP only for active labels.
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if int(inputs.shape[1]) != self.source_dim:
            raise ValueError(
                f"Expected M={self.source_dim}, got {int(inputs.shape[1])}"
            )
        ...
        # Active slice and MLP delta.
        x_active = inputs.index_select(dim=2, index=self.active_idx)  # (B, M, L_active)
        mean_active = mean_all.index_select(dim=1, index=self.active_idx)  # (B, L_active)
        ...
        # Stitch: inactive uses mean only.
...
def _delta_stats(model: NNSplitEnsembleModel, loader: torch.utils.data.DataLoader) -> tuple[float, float]:
    """
...
def main() -> None:
    ...
    model_name = f"torch_nn_split({','.join(e3)})"
    ...
    print(
        ...
        + " | ".join(
            f"{k} avg nnz/row={_csr_avg_nnz_per_row(p):.2f}" for k, p in zip(e3, train_preds, strict=True)
        )
    )
