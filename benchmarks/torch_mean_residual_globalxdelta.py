# STATUS: EXPERIMENTAL
# Purpose: Mean-like ensemble with multiplicative per-label residual weights around global weights.
#
# Variant: torch_mean_residual_globalxdelta (see EXPERIMENTS.md)
#
# Rationale:
# - Keep global per-model weights as the default behavior.
# - Make per-label adjustments *relative* to each model's global strength:
#
#     w_eff[m, l] = global_w[m] * (1 + delta_w[m, l])
#
# - Initialize delta_w = 0 so initial behavior is exactly global_w.
...
# Allow running as a script: `uv run benchmarks/torch_mean_residual_globalxdelta.py`
...
class MeanResidualGlobalXDeltaEnsemble(nn.Module):
    """
    Mean-like ensemble with multiplicative per-label residual weights around global weights.
    ...
    def __init__(self, n_models: int, n_labels: int, init_global: torch.Tensor | None):
        ...
        # In this variant, residuals are *multiplicative* via (1 + delta_w).
    ...
    def effective_w(self) -> torch.Tensor:
        """
        Effective per-model-per-label weights.

        Returns:
            Tensor of shape (M, L): global_w[:,None] * (1 + delta_w)
        """
        return self.global_w[:, None] * (1.0 + self.delta_w)
...
def main() -> None:
    ...
    model_name = f"torch_mean_residual_globalxdelta({','.join(ensemble_keys)})"
    ...
    model = MeanResidualGlobalXDeltaEnsemble(
