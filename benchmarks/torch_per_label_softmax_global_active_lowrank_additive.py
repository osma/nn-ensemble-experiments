...
# Simplification 7: Additive delta application instead of multiplicative.
#
# Replace multiplicative with additive delta application.
# Increase DELTA_GATE_MAX from 0.2 to 1.0.
...
class ActiveLowRankEnsemble(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Gate on the delta. max=1.0. Init p=0.1.
    ...
    def get_lowrank_delta(self, x_active: torch.Tensor, base_logits_active: torch.Tensor) -> torch.Tensor:
        ...
        DELTA_GATE_MAX = 1.0  # Increased for additive
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Simplification 7: Additive application
    ...
    *,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for xb, yb in train_loader:
            ...
            with torch.no_grad():
                gn_base = torch.nn.utils.clip_grad_norm_([model.g_raw, model.w_delta, model.bias], 1e9).item()
                gn_lowrank = torch.nn.utils.clip_grad_norm_(list(model.lowrank.parameters()) + [model.raw_gate], 1e9).item()
                grad_norm_base += gn_base
                grad_norm_lowrank += gn_lowrank
                n_batches += 1

            optimizer.step()
            last_loss = float(loss.item())

        avg_gn_base = grad_norm_base / max(1, n_batches)
        avg_gn_lowrank = grad_norm_lowrank / max(1, n_batches)
        ...
        print(
            ...
            f"gn_base={avg_gn_base:.4f} gn_lr={avg_gn_lowrank:.4f} | "
            f"gate={gate_val:.4f} LR_delta_mean={delta_mean_abs:.6f} p95={delta_p95_abs:.6f} | "
...
def main() -> None:
    ...
    model_name = f"torch_per_label_softmax_global_active_lowrank_additive({','.join(ensemble_keys)})"
