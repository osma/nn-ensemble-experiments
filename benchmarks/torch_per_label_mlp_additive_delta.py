...
#
# Variant: torch_per_label_mlp_additive_delta
#   Change ONLY the final combination rule on active labels:
#     - Baseline (multiplicative): out_active = base_active * (1 + gate * delta_active)
#     - This variant (additive):   out_active = base_active + gate * delta_active
#
...
# Allow running as a script: `uv run benchmarks/torch_per_label_mlp_additive_delta.py`
...
class TwoStagePerLabelMLPActive(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #  2) center per sample to limit global shifts and constrain to redistributive adjustments.
        ...
        # Additive residual in logit space: out_active = base_active + gate * delta
        out_active = base_logits_active + (gate * delta_active)
...
def main() -> None:
    ...
    model_name = f"torch_per_label_mlp_additive_delta({','.join(e3)})"
    ...
    # -----------------
    print("\n====================")
    print("Stage 2: residual MLP (active labels only) [ADDATIVE DELTA]")
    print("====================")
    ...
    for epoch in range(1, EPOCHS_STAGE2 + 1):
        ...
        with torch.no_grad():
            ...
            if model.n_active == 0:
                ...
                # Match forward(): center per sample before applying the gate.
                delta_dbg = delta_dbg - delta_dbg.mean(dim=1, keepdim=True)

                delta_stats = _tensor_stats(delta_dbg.detach().cpu())
                # "gated_delta" is the additive term added to base logits: (gate * delta)
        ...
        print(
            ...
            f"delta(tanh-bounded, centered): {_fmt_stats(delta_stats)} | gated_delta(add): {_fmt_stats(delta_g_stats)} | "
