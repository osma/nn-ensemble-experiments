...
#
# Follows the architecture of torch_per_label_softmax_global_active_mlp, but
# replaces the massive flattened MLP with a Low-Rank matrix factorization
# for the cross-label coupling, drastically reducing parameter count and
# maintaining the safety constraints (multiplicative gating, tanh bound, centering).
...
class ActiveLowRankEnsemble(nn.Module):
    ...
    def effective_w(self) -> torch.Tensor:
        return self.global_w()[:, None].expand(self.n_models, self.n_labels)
...
def train_and_evaluate(
    *,
    ...
    optimizer = optim.AdamW(
        [
            {"params": [model.g_raw, model.bias], "lr": BEST_LR, "weight_decay": BEST_WEIGHT_DECAY},
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        with torch.no_grad():
            ...
            lr_u_gnorm = grad_norms.get("lowrank.U", 0.0)
            lr_v_gnorm = grad_norms.get("lowrank.V", 0.0)
            lr_w_gnorm = grad_norms.get("lowrank.W", 0.0)
            gate_gnorm = grad_norms.get("raw_gate", 0.0)
        ...
        print(
            ...
            f"gnorm(U/V/W/gate)={lr_u_gnorm:.4f}/{lr_v_gnorm:.4f}/{lr_w_gnorm:.4f}/{gate_gnorm:.4f} | "
...
def main() -> None:
    ...
    model_name = f"torch_per_label_softmax_global_active_lowrank_no_delta({','.join(ensemble_keys)})"
