# STATUS: EXPERIMENTAL
# Purpose: Low-rank residual (U@V) + A1 mixing (P@Q) but with a nonlinearity:
#   mix = relu(s @ P) @ Q
#
# Still trained as probabilities with clamp + BCELoss.
...
class LowRankResidualMixReLU(nn.Module):
    def __init__(
        self,
        *,
        n_models: int,
...
def main() -> None:
    ...
    model_name = f"torch_lowrank_residual_mix_relu({','.join(ensemble_keys)})"
