# STATUS: EXPERIMENTAL
# Purpose: torch_mean_residual variant with a tiny per-model global bias term.
#
# Variant: torch_mean_residual_bias_per_model
#
# Change vs baseline:
#   Add bias_model[m] (M scalars) and apply it as an offset to each model's
#   input channel before weighting:
#
#     logits[b, l] = sum_m (w_global[m] + delta_w[m, l]) * (x[b, m, l] + bias_model[m]) + bias[l]
#
# Notes:
# - No new CLI parameters (kept identical to torch_mean_residual.py).
# - Regularization remains: L2 on delta_w and bias[l]. bias_model is intentionally
#   left unregularized to keep this a minimal-capacity tweak (3 scalars).
from __future__ import annotations

from pathlib import Path
import sys

# Allow running as a script: `uv run benchmarks/torch_mean_residual_bias_per_model.py`
...
class MeanResidualBiasPerModelEnsemble(nn.Module):
    """
    Mean-like ensemble with per-label residual weights and a per-model global bias.
    ...
    def __init__(self, n_models: int, n_labels: int, init_global: torch.Tensor | None):
        ...
        # Per-model global input bias (applied per-channel before weighting).
        self.bias_model = nn.Parameter(torch.zeros((n_models,), dtype=torch.float32))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_adj = x + self.bias_model.view(1, self.n_models, 1)  # (B, M, L)
        logits = (x_adj * w_eff.unsqueeze(0)).sum(dim=1) + self.bias  # (B, L)
        return logits
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "bias_model values, and output score distribution on the early-stop train subset."
        ),
    )
    ...
    model_name = f"torch_mean_residual_bias_per_model({','.join(ensemble_keys)})"
    ...
    model = MeanResidualBiasPerModelEnsemble(
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if print_delta or debug:
            with torch.no_grad():
                ...
                bias_model = model.bias_model.detach().cpu().numpy()
            ...
            diag = (
                ...
                f"bias_model=[{bias_model[0]:.3e},{bias_model[1]:.3e},{bias_model[2]:.3e}]"
            )
        ...
        if debug:
            with torch.no_grad():
                ...
                bias_model_t = model.bias_model.detach()
            ...
            bm_stats = _tensor_stats_1d(bias_model_t)
            ...
            extra = (
                ...
                f"    bias_model: mean={bm_stats['mean']:.6e} std={bm_stats['std']:.6e} "
                f"min={bm_stats['min']:.6e} p50={bm_stats['p50']:.6e} max={bm_stats['max']:.6e}\n"
