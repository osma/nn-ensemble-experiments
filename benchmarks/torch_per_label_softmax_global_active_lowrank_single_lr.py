...
# Simplification 5: single optimizer group with unified learning rate (1e-3).
#
# Follows the architecture of torch_per_label_softmax_global_active_lowrank, but
# uses a single optimizer group and a single learning rate for all parameters.
...
def train_and_evaluate(
    *,
    ...
    # Simplification 5: Single optimizer group for all parameters
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for xb, yb in train_loader:
            ...
            last_loss_l2 += float(loss_l2.item())
            n_batches += 1

        avg_loss_bce = last_loss_bce / n_batches
        avg_loss_l2 = last_loss_l2 / n_batches
        ...
        print(
            f"Epoch {epoch:02d} | loss_bce={avg_loss_bce:.6f} l2={avg_loss_l2:.6f} grad={total_norm:.4f} | "
...
def main() -> None:
    ...
    model_name = f"torch_per_label_softmax_global_active_lowrank_single_lr({','.join(ensemble_keys)})"
