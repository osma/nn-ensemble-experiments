# STATUS: EXPERIMENTAL
# Purpose: torch_per_label_mlp variant (Variant 4: layernorm feats).
...
#
# Variant change (controlled): add LayerNorm over the *flattened* stage-2 feature vector per sample
# (shape (C * L_active,)) before feeding into fc1.
#
# Everything else (data loading, preprocessing, gating, clamp, centering, early stop policy,
# optimizer/hparams) matches benchmarks/torch_per_label_mlp.py.
...
# Allow running as a script: `uv run benchmarks/torch_per_label_mlp_layernorm_feats.py`
...
class ResidualMLPActive(nn.Module):
    ...
    Variant change: LayerNorm over the flattened feature vector per sample (C * L_active).
    """
    ...
    def __init__(
        ...
        *,
        ...
        # LayerNorm on flattened features (affine) — controlled change vs baseline.
        # Note: for n_active==0 we skip computation in forward(), so normalized_shape can be 0 here
        # without being used; however, nn.LayerNorm requires normalized_shape > 0. To keep this robust,
        # we create a no-op Identity when n_active==0.
        n_flat = self.n_channels * self.n_active
        self.feat_norm: nn.Module = (
            nn.LayerNorm(n_flat, elementwise_affine=True) if n_flat > 0 else nn.Identity()
        )
    ...
    def reset_parameters(self) -> None:
        ...
        # LayerNorm default init is (weight=1, bias=0), which is appropriate.
...
def main() -> None:
    ...
    model_name = f"torch_per_label_mlp_layernorm_feats({','.join(e3)})"
    ...
    # -----------------
    print("\n====================")
    print("Stage 2: residual MLP (active labels only) + LayerNorm(feats)")
    print("====================")
    ...
    for epoch in range(1, EPOCHS_STAGE2 + 1):
        ...
        with torch.no_grad():
            ...
            if model.n_active == 0:
                ...
                flat_dbg = model.residual.feat_norm(flat_dbg)
