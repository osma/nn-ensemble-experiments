# STATUS: EXPERIMENTAL
# Purpose: Variant of torch_per_label_mlp that removes the per-sample centering step on delta_active.
#
# Baseline (torch_per_label_mlp) computes a tanh-bounded residual delta and then enforces:
#   delta_active = delta_active - mean(delta_active over active labels)
# This variant removes that centering constraint to allow uniform boosts/suppressions of active logits.
#
# All other policies intentionally match torch_per_label_mlp:
#   - Stage 1: torch_per_label trained with its standard early stopping.
#   - Stage 2: active-label-only residual MLP with:
#       * zero-initialized final layer (near-no-op start)
#       * bounded learnable scalar gate in (0, DELTA_GATE_MAX)
#       * tanh bounding (DELTA_CLAMP)
#       * multiplicative application: out_active = base_active * (1 + gate * delta_active)
#
# Debug:
...
# Allow running as a script: `uv run benchmarks/torch_per_label_mlp_remove_centering.py`
...
class TwoStagePerLabelMLPActive(nn.Module):
    ...
    Variant behavior: NO per-sample centering of delta_active.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # In the baseline torch_per_label_mlp, this delta is centered per sample.
        # This variant intentionally does NOT center the delta.
...
def main() -> None:
    ...
    model_name = f"torch_per_label_mlp_remove_centering({','.join(e3)})"
    ...
    # -----------------
    print("\n====================")
    print("Stage 2: residual MLP (active labels only) [NO CENTERING]")
    print("====================")
    ...
    for epoch in range(1, EPOCHS_STAGE2 + 1):
        ...
        with torch.no_grad():
            ...
            if model.n_active == 0:
                ...
                # NOTE: Unlike baseline torch_per_label_mlp, this variant does NOT center delta_dbg.
        ...
        print(
            ...
            f"delta(tanh-bounded, NOT centered): {_fmt_stats(delta_stats)} | gated_delta(mult): {_fmt_stats(delta_g_stats)} | "
