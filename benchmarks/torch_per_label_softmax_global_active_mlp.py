# STATUS: EXPERIMENTAL
# Purpose: torch_per_label variant with softmax-constrained global weights + active label MLP.
from pathlib import Path
import sys
import time

# Allow running as a script: `uv run benchmarks/torch_per_label_softmax_global_active_mlp.py`
...
class ActiveMLPEnsemble(nn.Module):
    def __init__(
        self,
        *,
        n_models: int,
        ...
        # MLP for active labels. Input size is (M + 1) * n_active (includes base_logits_active)
        self.flatten = nn.Flatten()
        self.dropout1 = nn.Dropout(dropout_rate)
        self.hidden = nn.Linear((self.n_models + 1) * self.n_active, hidden_dim)
        self.dropout2 = nn.Dropout(dropout_rate)
        self.delta_layer = nn.Linear(hidden_dim, self.n_active)

        # Gate on the MLP. delta_gate_init=0.02, max=0.2. p = 0.02 / 0.2 = 0.1.
        raw_init = float(np.log(0.1 / 0.9))
        self.raw_gate = nn.Parameter(torch.tensor(raw_init, dtype=torch.float32))

        # Initialize MLP delta layer at 0 to start as a near-no-op
    ...
    def get_mlp_delta(self, x_active: torch.Tensor, base_logits_active: torch.Tensor) -> torch.Tensor:
        feats = torch.cat([x_active, base_logits_active.unsqueeze(1)], dim=1)  # (B, M+1, L_active)
        x_flat = self.flatten(feats)
        h = self.dropout1(x_flat)
        h = F.relu(self.hidden(h))
        h = self.dropout2(h)
        delta_active = self.delta_layer(h)  # (B, L_active)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        gated_delta_active = self.get_mlp_delta(x_active, base_logits_active)
...
def _delta_stats(model: ActiveMLPEnsemble, loader: torch.utils.data.DataLoader, device: torch.device) -> tuple[float, float]:
    """Compute mean(|delta|) and p95(|delta|) over a dataset loader."""
    if model.n_active == 0:
        return 0.0, 0.0
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            delta = model.get_mlp_delta(x_active, base_logits_active)
...
def train_and_evaluate(
    *,
    ...
    # Separate optimizers for base and MLP components
    optimizer = optim.AdamW(
        [
            {"params": [model.g_raw, model.w_delta, model.bias], "lr": BEST_LR, "weight_decay": 0.0},
            {"params": [model.hidden.weight, model.hidden.bias, model.delta_layer.weight, model.delta_layer.bias, model.raw_gate], "lr": MLP_LR, "weight_decay": MLP_WEIGHT_DECAY},
        ],
        eps=1e-8,
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            ...
            f"gate={gate_val:.4f} MLP_delta_mean={delta_mean_abs:.6f} p95={delta_p95_abs:.6f} | "
...
def main() -> None:
    ...
    model_name = f"torch_per_label_softmax_global_active_mlp({','.join(ensemble_keys)})"
