# STATUS: EXPERIMENTAL
# Purpose: NN ensemble with low-rank MLP (factorized weights).
# Uses rank=32 for the low-rank approximation.
...
class LowRankNNModel(nn.Module):
    def __init__(
        self,
        source_dim: int,
        input_dim: int,
        ...
        # Low-rank factorization: U (input_dim*source_dim x rank), V (rank x output_dim)
        self.U = nn.Linear(input_dim * source_dim, rank, bias=False)
        self.V = nn.Linear(rank, output_dim, bias=True)
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        x = self.U(x)
        x = F.relu(x)
        x = self.dropout2(x) if hasattr(self, 'dropout2') else x
        delta = self.V(x)
...
def main() -> None:
    ...
    model_name = f"torch_nn_lowrank({','.join(e3)})"
    ...
    # Add dropout layer for second position
